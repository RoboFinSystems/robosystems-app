Create a GitHub issue for the robosystems-app repository based on the user's input.
...
2. **Gather Context** - If the user provides a file path or references existing code:
   ...
   - Check related components in `/components/`
   - Review any related API routes in `/app/api/`
...
6. **Set Issue Type** - After creation, set the issue type via GraphQL:
   ```bash
   # Get issue ID
   gh api graphql -f query='{ repository(owner: "RoboFinSystems", name: "robosystems-app") { issue(number: NUMBER) { id } } }'
...
[Read /components/ThemeProvider.tsx to understand current state]
