...
# Deploy current branch or tag to specified environment for RoboSystems App Next.js App
...
case "$ENVIRONMENT" in
    staging)
        ...
        TARGET_URL="https://staging.robosystems.ai"
        ;;
    prod)
        WORKFLOW_FILE="prod.yml"
        TARGET_URL="https://robosystems.ai"
        ;;
esac
...
# Verify this is the RoboSystems App app
APP_NAME=$(node -p "require('./package.json').name" 2>/dev/null || echo "unknown")
if [ "$APP_NAME" != "robosystems-app" ]; then
    print_warning "App name is '$APP_NAME', expected 'robosystems-app'. Continuing anyway..."
fi
